"""NeuroTechX-Atlas imagined-speech section: BCI Competition 2020 Track 3
(HF braindecode/bcic2020-3). New TASK DOMAIN (imagined speech, not sleep/
epilepsy/BCI-motor/affect/workload) and one of OpenEEGBench's 12.

15 subjects, 5-class imagined-word classification, 64-channel extended
10-20 montage, 200 Hz, 3s windows (600 samples). Pulled via braindecode's
own `BaseConcatDataset.pull_from_hub` -- shares atlas_hf_braindecode.py's
grouping helper with Arithmetic and MDD.
"""
...
def _reve_channel_names(ch_names):
...
def make_embed_fn(model, sfreq, dev, ch_names):
    ...
    if model == "reve":
        ...
        ch = _reve_channel_names(ch_names)
        return lambda X: embed_reve(X, ch, sfreq, dev)
    raise SystemExit(f"unknown model {model!r} (bci2020-3 supports logbandpower, reve)")
...
def main():
    ...
    ds = hb.pull(HF_ID)
    ch_names = list(ds.datasets[0].windows.info["ch_names"])
    groups = hb.concat_dataset_to_subject_groups(ds)
    tr_s, dv_s, ev_s = hb.subject_splits(groups.keys())
    print(f"[bci2020-3] model={a.model} subjects train/dev/eval="
          f"{len(tr_s)}/{len(dv_s)}/{len(ev_s)}", flush=True)
    embed_fn = make_embed_fn(a.model, a.sfreq, dev_, ch_names)
    splits = {}
    ...
    for erase in configs:
        ...
        print(f"\n[bci2020-3] {a.model}{tag} 5-class imagined speech:")
    ...
    if False in results and True in results:
        ...
        print(f"\n[bci2020-3] {a.model} identity-free Δ(eval kappa) = {d:+.3f} "
