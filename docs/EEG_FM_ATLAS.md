...
| **Sleep** | `atlas_sleep_section.py` + `atlas_sleep_hmc.py` + `atlas_sleep_dod.py` + `atlas_sleep_eareeg.py` + `atlas_sleep_isruc.py` | Sleep-EDF Expanded (197 recs) + **HMC** (151 recs, NeuroAtlas-named) + **Dreem DOD-H/DOD-O** (25+56 subj) + **Ear-EEG** (ds005178, 10 subj, wearable 4-ch modality) + **ISRUC-Sleep** (subgroups I/II/III, 84/17/17 subj, NeuroAtlas-named) | Balanced-acc + Cohen's κ (5-class AASM) + patient-id-free Δ | ✅ **HMC = cleanest identity-trap confirmation yet: REVE WINS on raw perf (κ 0.898 > classical 0.859) yet COLLAPSES hardest under identity erasure (Δ+0.490 vs classical Δ+0.085)**. Sleep-EDF: classical κ 0.551→0.508 (Δ+0.043); REVE 0.459→0.202 (Δ+0.257). **Dreem DOD REVERSES the direction**: classical shows the larger κ drop on both DOD-H (Δ+0.233 vs REVE Δ+0.121) and DOD-O (Δ+0.165 vs REVE Δ+0.100). **Ear-EEG (new acquisition-modality axis) also shows classical trapping harder** (Δ+0.093 vs REVE Δ+0.014). **ISRUC (largest N besides Sleep-EDF/HMC, κ 0.55-0.64) shows a small, nearly-equal Δ for both** (classical +0.064, REVE +0.077) — the 6-dataset sleep record is now genuinely mixed (3 REVE-worse, 2 classical-worse, ISRUC roughly tied), reported as-is, not smoothed into a universal claim. `docs/sleep_edf_results.md`, `docs/hmc_sleep_results.md`, `docs/dreem_dod_sleep_results.md`, `docs/eareeg_sleep_results.md`, `docs/isruc_sleep_results.md` |
