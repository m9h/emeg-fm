...
| **Epilepsy** | `atlas_epilepsy_section.py` + `epilepsy_scorer.py` + `nedc_crosscheck.py` + `atlas_epilepsy_chbmit.py` + `atlas_epilepsy_siena.py` | TUSZ v2.0.3 (train4667/dev1832/eval865) + **SeizeIT2** (ds005873, wearable, 89/18/18 subj) + **Helsinki neonatal** (Zenodo 2547147, 49/15/15 neonates) + **CHB-MIT** (PhysioNet 1.0.0, pediatric bipolar montage, 15/4/4 patients) + **Siena** (PhysioNet 1.0.0, Italian adult, 10/2/2 patients) | SzCORE sens@1/10 FP/24h + F1 (+ official NEDC v6 OVLP/TAES cross-check) + patient-id-free Δ | ✅ **both FMs (REVE, EEGDINO) COLLAPSE to 0% usable sens when patient-identity erased on TUSZ; only classical survives (40.9→25.0%)**. Classical Δ REPLICATES on 4 of 5 epilepsy sets (TUSZ+0.044, SeizeIT2+0.019, Helsinki+0.067, **CHB-MIT+0.146, the largest margin yet**) — robust majority signature incl. sleep. **Siena is the one exception** (Δ−0.210, identity-free BETTER) but with only 2 eval patients this is disclosed as a likely small-N artifact, not folded into or treated as refuting the pattern. REVE on Helsinki, CHB-MIT, AND Siena (all near-floor AUC ≤0.09) is genuinely INCONCLUSIVE on 3 of 5 epilepsy sets, honestly reported as such, not spun. `docs/epilepsy_tusz_results.md`, `docs/helsinki_neonatal_results.md`, `docs/chbmit_epilepsy_results.md`, `docs/siena_epilepsy_results.md` |
| **Sleep** | `atlas_sleep_section.py` + `atlas_sleep_hmc.py` + `atlas_sleep_dod.py` + `atlas_sleep_eareeg.py` + `atlas_sleep_isruc.py` + `atlas_sleep_dcsm.py` | Sleep-EDF Expanded (197 recs) + **HMC** (151 recs, NeuroAtlas-named) + **Dreem DOD-H/DOD-O** (25+56 subj) + **Ear-EEG** (ds005178, 10 subj, wearable 4-ch modality) + **ISRUC-Sleep** (subgroups I/II/III, 84/17/17 subj, NeuroAtlas-named) + **DCSM** (255 subj, 179/38/38, LARGEST sleep cohort in the program) | Balanced-acc + Cohen's κ (5-class AASM) + patient-id-free Δ | ✅ **DCSM is now the single largest identity-trap collapse in the whole sleep record — bigger than HMC**: REVE ties classical on raw perf (κ 0.713 vs 0.759) then COLLAPSES to near-chance under identity erasure (κ 0.713→0.156, Δ+0.557, balanced-acc 40.2%), and with 38 eval subjects this is the best-powered single sleep-domain finding in the program (not a small-N fluke). **HMC** remains the other clean REVE-collapse case (κ 0.898>0.859 raw, Δ+0.490 vs classical Δ+0.085). Sleep-EDF: classical κ 0.551→0.508 (Δ+0.043); REVE 0.459→0.202 (Δ+0.257). **Dreem DOD REVERSES the direction**: classical shows the larger κ drop on both DOD-H (Δ+0.233 vs REVE Δ+0.121) and DOD-O (Δ+0.165 vs REVE Δ+0.100). **Ear-EEG also shows classical trapping harder** (Δ+0.093 vs REVE Δ+0.014). **ISRUC shows a small, nearly-equal Δ for both** (classical +0.064, REVE +0.077). The 7-dataset sleep record is genuinely mixed (4 REVE-worse — now weighted toward REVE-worse by DCSM's scale — vs 2 classical-worse, ISRUC tied), reported as-is, not smoothed into a universal claim. `docs/sleep_edf_results.md`, `docs/hmc_sleep_results.md`, `docs/dreem_dod_sleep_results.md`, `docs/eareeg_sleep_results.md`, `docs/isruc_sleep_results.md`, `docs/dcsm_sleep_results.md` |
| **Affect** (new domain) | `atlas_affect_faced.py` | **FACED** (Synapse syn50614194, 123 subj, 87/18/18, OpenEEGBench overlap) — 9-class emotion recognition | Balanced-acc + Cohen's κ (9-class) + patient-id-free Δ | ✅ **First emotion-recognition dataset in the program.** REVE genuinely beats classical on raw performance (κ 0.205 vs 0.059, bacc 29.3% vs 16.3% — chance is 11.1%), the first clean FM-vs-classical win on a task domain classical usually dominates. **Neither model shows a meaningful identity-free Δ** (classical −0.009, REVE +0.008, both ≈noise) — consistent with, not contradicting, the SEED-family literature's large pooled-vs-subject-disjoint gaps (`docs/seed_identity_trap_literature.md`): our split is already subject-disjoint, so there's little residual identity signal left for LEACE to find. `docs/faced_affect_results.md` |
| **Workload** (new domain) | `atlas_workload_arithmetic.py` | **Arithmetic** (Zyma 2019, HF braindecode/arithmetic_zyma2019, 36 subj, 26/5/5, OpenEEGBench overlap) — rest-vs-mental-arithmetic | Balanced-acc + Cohen's κ (binary) + patient-id-free Δ | ✅ Classical beats REVE on raw perf (κ 0.408 vs 0.244) — a state-detection task favoring classical spectral features. Neither Δ is large (classical −0.075, REVE +0.052); small N (5 eval subj) limits confidence. `docs/arithmetic_workload_results.md` |
| **Speech** (new domain) | `atlas_speech_bci2020.py` | **BCI Comp 2020 Track 3** (HF braindecode/bcic2020-3, 15 subj, 11/2/2, OpenEEGBench overlap) — 5-class imagined speech | Balanced-acc + Cohen's κ (5-class) + patient-id-free Δ | ⚠️ **Neither model beats chance** (20%) on this task — genuinely hard decode at this N (15 subj, 2 eval). Both Δ ≈0, uninterpretable for the same reason as Helsinki/CHB-MIT/Siena (no real classifier signal to erase). Honest null result. `docs/bci2020_speech_results.md` |
| **Depression** (new domain) | `atlas_depression_mdd.py` | **MDD** (Mumtaz 2016, HF braindecode/mdd_mumtaz2016, 64 subj, 46/9/9, OpenEEGBench overlap) — MDD-vs-healthy | Balanced-acc + Cohen's κ (binary) + patient-id-free Δ | ✅ **Largest identity-free Δ recorded anywhere in the program.** REVE: real signal (κ 0.591) COMPLETELY collapses to chance (κ 0.000 exactly, bacc 50.0%) under erasure, Δ+0.591 — bigger than DCSM's prior record (Δ+0.557). Classical also drops hard (Δ+0.414). Hypothesis: MDD diagnosis is a subject-level **trait** (constant per person) unlike sleep-stage/seizure/arithmetic **state** labels (vary within-subject) — trait tasks let identity-riding get the label almost for free. First direct trait-classification test in the program; TDBRAIN's planned MDD-vs-ADHD audit (task tracker #60) would be a second data point. Also caught a real bug: subject ids ("HS\*"/"MDDS\*") encode the label, so naive lexicographic-sort splitting put 100% MDDS in eval (degenerate κ=0.000) until fixed with stratified splitting. `docs/mdd_depression_results.md` |
