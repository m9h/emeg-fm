...
| **Epilepsy** | `atlas_epilepsy_section.py` + `epilepsy_scorer.py` + `nedc_crosscheck.py` + `atlas_epilepsy_chbmit.py` | TUSZ v2.0.3 (train4667/dev1832/eval865) + **SeizeIT2** (ds005873, wearable, 89/18/18 subj) + **Helsinki neonatal** (Zenodo 2547147, 49/15/15 neonates) + **CHB-MIT** (PhysioNet 1.0.0, pediatric bipolar montage, 15/4/4 patients) | SzCORE sens@1/10 FP/24h + F1 (+ official NEDC v6 OVLP/TAES cross-check) + patient-id-free Δ | ✅ **both FMs (REVE, EEGDINO) COLLAPSE to 0% usable sens when patient-identity erased on TUSZ; only classical survives (40.9→25.0%)**. Classical Δ REPLICATES on all 4 epilepsy sets (TUSZ+0.044, SeizeIT2+0.019, Helsinki+0.067, **CHB-MIT+0.146, the largest margin yet**) — robust 5-domain signature incl. sleep. REVE on Helsinki (small N=15 eval) AND CHB-MIT (bipolar-montage anchor-electrode approximation collapses to near-floor AUC 0.01-0.09) is genuinely INCONCLUSIVE, honestly reported as such, not spun. `docs/epilepsy_tusz_results.md`, `docs/helsinki_neonatal_results.md`, `docs/chbmit_epilepsy_results.md` |
